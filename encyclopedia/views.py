...
def entry_by_title(request, title):
    ...
    if request.method == "POST":
        ...
        return render(
            request, "encyclopedia/view_entry.html", {"entry": entry, "title": title}
        )
...
def search(request):
    if request.method == "POST":
        ...
        if search_entry.lower() in [title.lower() for title in util.list_entries()]:
            ...
            if to_display:
                return render(
                    ...
                    {"entries": to_display, "search": True},
                )
            else:
                return entry_by_title(request, title=search_entry)
    else:
